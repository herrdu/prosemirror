...
import { NodeSpec, AttributeSpec, SchemaSpec, MarkSpec } from "./types";
...
export class NodeType<S extends Schema = any> {
   ...
   * The attributes that nodes of this type get.
   */
  defaultAttrs?: { [name: string]: any } | null;
  ...
  hasRequiredAttrs(ignore?: string[]) {
    for (let n in this.attrs) if (this.attrs[n].isRequired && (!ignore || !(n in ignore))) return true;
    return false;
  }

  compatibleContent(other: NodeSpec) {
...
export class MarkType<S extends Schema = any> {
  ...
  constructor(name: string, rank: any, schema: S, spec: MarkSpec) {
    // :: string
    // The name of the mark type.
    this.name = name;
  ...
  create(attrs?: { [key: string]: any } | null): Mark {
    if (!attrs && this.instance) return this.instance;
    return new Mark(this, computeAttrs(this.attrs, attrs));
  }

  static compile(marks: any, schema: Schema) {
    let result = Object.create(null),
      rank = 0;
    marks.forEach((name: string, spec: MarkSpec) => (result[name] = new MarkType(name, rank++, schema, spec)));
    return result;
  }
...
export class Schema<N extends string = any, M extends string = any> {
  ...
  node(type: string | NodeType, attrs?: { [key: string]: any }, content?: Fragment | Node | Node[], marks?: Mark[]) {
