import { Selection, NodeSelection, TextSelection, EditorState } from "../state";
import { dropPoint } from "../transform";
import { Slice, Node as ProsemirrorNode, ResolvedPos } from "../model";
...
const handlers: { [key: string]: (view: EditorView, event: any) => void } = {},
  editHandlers: { [key: string]: (view: EditorView, event: any) => void } = {};
...
export function initInput(view: EditorView) {
  ...
  view.domObserver = new DOMObserver(view, (from: number, to: number, typeOver: any, added: any) =>
...
function setSelectionOrigin(view: EditorView, origin: any) {
...
export function destroyInput(view: EditorView) {
  ...
  for (let type in view.eventHandlers) view.dom.removeEventListener(type, view.eventHandlers[type]);
...
function runCustomHandler(view: EditorView, event: any) {
...
export function dispatchEvent(view: EditorView, event: any) {
  if (!runCustomHandler(view, event) && handlers[event.type] && (view.editable || !(event.type in editHandlers)))
    handlers[event.type](view, event);
}

editHandlers.keydown = (view: EditorView, event: any) => {
  ...
  if (browser.ios && event.keyCode == 13 && !event.ctrlKey && !event.altKey && !event.metaKey) {
    ...
    view.lastIOSEnterFallbackTimeout = window.setTimeout(() => {
...
function isNear(event: MouseEvent, click: EditorView["lastClick"]) {
  let dx = click.x - event.clientX,
    dy = click.y - event.clientY;
  return dx * dx + dy * dy < 100;
}

function runHandlerOnContext(view: EditorView, propName: string, pos: number, inside: number, event: MouseEvent) {
...
function defaultTripleClick(view: EditorView, inside: number) {
...
handlers.mousedown = (view: EditorView, event: MouseEvent) => {
  ...
  if (type == "singleClick") view.mouseDown = new MouseDown(view, pos, event, flushed);
...
class MouseDown {
  ...
  constructor(view: EditorView, pos: any, event: MouseEvent, flushed?: boolean) {
    this.view = view;
    ...
    let targetNode: any, targetPos: any;
    if (pos.inside > -1) {
    ...
    if (this.target && this.mightDrag && (this.mightDrag.addAttr || this.mightDrag.setUneditable)) {
      ...
      if (this.mightDrag.setUneditable) setTimeout(() => this.target.setAttribute("contentEditable", "false"), 20);
...
function inOrNearComposition(view: EditorView, event: MouseEvent) {
...
function scheduleComposeEnd(view: EditorView, delay: number) {
  clearTimeout(view.composingTimeout);
  if (delay > -1) view.composingTimeout = window.setTimeout(() => endComposition(view), delay);
}

export function endComposition(view: EditorView, forceUpdate?: boolean) {
  view.domObserver.forceFlush();
  view.composing = false;
...
function capturePaste(view: EditorView, e: ClipboardEvent) {
  if (!view.dom.parentNode) return;
  ...
  setTimeout(() => {
    ...
    if (plainText) doPaste(view, (target as HTMLTextAreaElement).value, null, e);
    else doPaste(view, target.textContent, target.innerHTML, e);
  }, 50);
}

function doPaste(view: EditorView, text: string, html: string, e: ClipboardEvent) {
...
editHandlers.paste = (view, e) => {
  let data = brokenClipboardAPI ? null : e.clipboardData;
  let html = data && data.getData("text/html"),
...
export class Dragging {
  ...
  constructor(slice: Slice, move: boolean) {
    this.slice = slice;
    this.move = move;
  }
}
...
handlers.dragstart = (view: EditorView, e: any) => {
  ...
  if (pos && pos.pos >= sel.from && pos.pos <= (sel instanceof NodeSelection ? sel.to - 1 : sel.to)) {
    ...
    if (!desc || !desc.node.type.spec.draggable || desc == (view.docView as any)) return;
...
editHandlers.drop = (view, e) => {
  ...
  e.preventDefault();
  let insertPos = slice ? dropPoint(view.state.doc as any, $mouse.pos, slice) : $mouse.pos;
  ...
  if (
    ...
    tr.setSelection(new NodeSelection($pos as any));
  else tr.setSelection(selectionBetween(view, $pos, tr.doc.resolve(tr.mapping.map(insertPos))));
  view.focus();
