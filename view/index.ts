export * from "./types";
import { NodeSelection, EditorState, Transaction, Selection } from "../state";
...
export class EditorView<S extends Schema = any> {
  state: EditorState<S>;
  _props: DirectEditorProps<S>;
  _root: Document | DocumentFragment;

  focused: boolean;
  trackWrites: null | Node;
  ...
  lastSelectedViewDesc: NodeViewDesc;
  ...
  lastKeyCode: null | number = null;
  lastKeyCodeTime: null | number = null;
  lastClick: { time: number; x: number; y: number; type: string };
  lastSelectionOrigin: null;
  lastSelectionTime: number;
  lastIOSEnter: number;
  lastIOSEnterFallbackTimeout: number;
  composingTimeout: number | null;
  ...
  // to the document.
  constructor(
    place: HTMLElement | ((p: HTMLElement) => void) | { mount: HTMLElement } | undefined,
    ...
    if (place) {
      if (place instanceof HTMLElement && place.appendChild) place.appendChild(this.dom);
      else if (place instanceof Function && place.apply) place(this.dom);
  ...
  get props() {
    if (this._props.state != this.state) {
      ...
      this._props = { state: undefined, dispatchTransaction: undefined };
  ...
  setProps(props: DirectEditorProps) {
    let updated: DirectEditorProps = { state: undefined, dispatchTransaction: undefined };
  ...
  updateStateInner(state: EditorState, reconfigured: boolean) {
    let prev = this.state,
      redraw = false;
    this.state = state;
    if (reconfigured) {
    ...
    let oldScrollPos =
      scroll == "preserve" && updateSel && this.dom.style.overflowAnchor == null && storeScrollPos(this);
    ...
    if (scroll == "reset") {
      ...
      if (this.someProp("handleScrollToSelection", (f) => f(this))) {
        ...
        scrollRectIntoView(
          this,
          this.docView.domAfterPos(state.selection.from).getBoundingClientRect(),
          startDOM as HTMLElement
        );
      else scrollRectIntoView(this, this.coordsAtPos(state.selection.head), startDOM as HTMLElement);
  ...
  // directly).
  someProp(propName: string, f?: (prop: any) => any) {
  ...
  hasFocus() {
    return (this.root as Document).activeElement == this.dom;
  }
    ...
    if (cached == null)
      for (let search = this.dom.parentNode; search; search = search.parentNode) {
        if (search.nodeType == 9 || (search.nodeType == 11 && (search as any).host)) {
          if (!(search as any).getSelection) Object.getPrototypeOf(search).getSelection = () => document.getSelection();
          return (this._root = search as Document | DocumentFragment);
        }
      }
  ...
  posAtCoords(coords: ICoords): undefined | { pos: number; inside: number } {
  ...
  posAtDOM(node: Node, offset?: number, bias = -1) {
  ...
  endOfTextblock(dir: IDir, state?: EditorState): boolean {
...
function updateCursorWrapper(view: EditorView) {
  if (view.markCursor) {
    ...
    view.cursorWrapper = {
      dom,
      deco: Decoration.widget(view.state.selection.head, dom, { raw: true, marks: view.markCursor }),
    };
  } else {
...
function getEditable(view: EditorView) {
  return !view.someProp("editable", (value: (state: EditorState) => boolean) => value(view.state) === false);
}
