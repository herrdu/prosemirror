...
function skipIgnoredNodesLeft(view: EditorView) {
  ...
  if (browser.gecko && node.nodeType == 1 && offset < nodeLen(node) && isIgnorable(node.childNodes[offset]))
    force = true;
  for (;;) {
    if (offset > 0) {
      if (node.nodeType != 1) {
...
function skipIgnoredNodesRight(view: EditorView) {
  ...
  let moveNode: Node, moveOffset: number;
  for (;;) {
    if (offset < len) {
...
function setSelFocus(view: EditorView, sel: any, node: Node, offset: number) {
...
function switchEditable(view: EditorView, node: HTMLElement, state: any) {
...
function chromeDownArrowBug(view: EditorView) {
  ...
  if (
    ...
    switchEditable(view, child as HTMLElement, true);
    setTimeout(() => switchEditable(view, child as HTMLElement, false), 20);
  }
  return false;
}
