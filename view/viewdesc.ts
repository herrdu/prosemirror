import { DOMSerializer, Fragment, Mark, Node as ProsemirrorNode, ParseRule } from "../model";
...
export class ViewDesc {
  ...
  constructor(parent?: ViewDesc, children?: ViewDesc[], dom?: Node, contentDOM?: Node | null) {
    this.parent = parent;
  ...
  // widget/mark/node.
  matchesWidget(_deco: Decoration) {
    return false;
  }
  matchesMark(_type: Mark) {
    return false;
  }
  matchesNode(_node: ProsemirrorNode, _outerDeco: Decoration[], _innerDeco: DecorationSet) {
    return false;
  }
  matchesHack() {
    return false;
  }
  slice(_from: number, _to: number, _view: EditorView): MarkViewDesc | undefined {
    return undefined;
  }
  ...
  // parse them.
  parseRule(): ParseRule | null | undefined {
    return null;
  }
  ...
  destroy() {
    this.parent = null;
    if (this.dom && this.dom.pmViewDesc === this) {
  ...
  localPosFromDOM(dom: Node, offset: number, bias?: number): number {
    ...
    if (this.contentDOM && this.contentDOM.contains(dom.nodeType == 1 ? dom : dom.parentNode)) {
      if (bias && bias < 0) {
        ...
        let desc: ViewDesc | null | undefined;
        if (dom == this.contentDOM) {
          domBefore = dom.childNodes[offset - 1];
        } else {
          while (dom.parentNode != this.contentDOM) {
            dom = dom.parentNode as any;
          }
    ...
    let atEnd: boolean | null | number | undefined;
    ...
    return (atEnd == null ? bias && bias > 0 : atEnd) ? this.posAtEnd : this.posAtStart;
  }
  ...
  domFromPos(pos: number): { node: Node; offset: number } {
    ...
    for (let offset = 0, i = 0; ; i++) {
      if (offset == pos) {
        while (
          i < (this as any).children.length &&
          ((this as any).children[i].beforePosition || (this as any).children[i].dom.parentNode != this.contentDOM)
        )
          i++;
        return {
          ...
          offset:
            i == (this as any).children.length
              ...
              : domIndex((this as any).children[i].dom),
        };
      }
      if (i == (this as any).children.length) throw new Error("Invalid position " + pos);
      let child = (this as any).children[i],
        end = offset + child.size;
  ...
  parseRange(from: number, to: number, base: number = 0) {
    if ((this as any).children.length == 0)
      return { node: this.contentDOM, from, to, fromOffset: 0, toOffset: (this as any).contentDOM.childNodes.length };
    ...
    for (let offset = base, i = 0; ; i++) {
      ...
      if (fromOffset == -1 && from <= end) {
        ...
        if (
          ...
          child.node &&
          child.contentDOM &&
          (this as any).contentDOM.contains(child.contentDOM)
        )
          return child.parseRange(from, to, childBase);

        from = offset;
        for (let j = i; j > 0; j--) {
          let prev = (this as any).children[j - 1];
      ...
      if (fromOffset > -1 && (end > to || i == (this as any).children.length - 1)) {
        to = end;
        for (let j = i + 1; j < (this as any).children.length; j++) {
          let next = (this as any).children[j];
        ...
        if (toOffset == -1) toOffset = (this as any).contentDOM.childNodes.length;
        break;
      }
      offset = end;
    }
  ...
  emptyChildAt(side: number) {
    if (this.border || !this.contentDOM || !(this as any).children.length) return false;
    let child = (this as any).children[side < 0 ? 0 : (this as any).children.length - 1];
...
class WidgetViewDesc extends ViewDesc {
  ...
  constructor(parent: ViewDesc, widget: Decoration, view: EditorView, pos: number) {
    ...
    if (typeof dom == "function")
      dom = dom(view, () => {
        if (!self) return pos;
        if (self.parent) return self.parent.posBeforeChild(self as any);
      });
...
class MarkViewDesc extends ViewDesc {
  ...
  slice(from: number, to: number, view: EditorView) {
    ...
    for (let i = 0; i < nodes!.length; i++) (nodes as any)[i].parent = copy;
    copy.children = nodes;
    return copy;
  }
}
...
export class NodeViewDesc extends ViewDesc {
  ...
  static create(
    ...
    if (node.isText) {
      ...
      ({ dom, contentDOM } = DOMSerializer.renderSpec(document, (node as any).type.spec.toDOM(node)));
    }
  ...
  parseRule() {
    ...
    let rule: ParseRule = { node: (this as any).node.type.name, attrs: (this as any).node.attrs };
    if (this?.node?.type.spec.code) rule.preserveWhitespace = "full";
    if (this.contentDOM && !this.contentLost) rule.contentElement = this.contentDOM as any;
    else (rule as any).getContent = () => (this.contentDOM ? Fragment.empty : this?.node?.content);
    return rule;
  }
