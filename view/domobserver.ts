...
export class DOMObserver {
  view: EditorView;
  handleDOMChange: (from: number, to: number, typeOver: boolean, added: any[]) => void;
  queue: any[];
  flushingSoon: number;
  observer: MutationObserver;
  currentSelection: SelectionState;
  onCharData: (e: MutationEvent) => void;
  suppressingSelectionUpdates: boolean;

  constructor(view: EditorView, handleDOMChange: (from: number, to: number, typeOver: boolean, added: any[]) => void) {
    this.view = view;
    ...
    if (useCharData) {
      this.onCharData = (e: MutationEvent) => {
