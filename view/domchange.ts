import { Fragment, DOMParser, ResolvedPos, Mark, Node as ProsemirrorNode } from "../model";
import { Selection, TextSelection, Transaction } from "../state";
...
function ruleFromNode(dom: HTMLElement) {
...
export function readDOMChange(view: EditorView, from: number, to: number, typeOver: boolean, addedNodes: Node[]) {
  if (from < 0) {
  ...
  let preferredPos: number, preferredSide: "end" | "start";
  ...
  let tr: Transaction,
    storedMarks: Array<Mark> | null | undefined,
    markChange: {
      mark: any;
      type: any;
    },
...
function resolveSelection(view: EditorView, doc: ProsemirrorNode, parsedSel: Selection) {
...
function isMarkChange(cur: Fragment, prev: Fragment) {
  ...
  let added = curMarks,
    ...
    type: string,
    mark: Mark,
    update: (node: ProsemirrorNode) => ProsemirrorNode;
...
function looksLikeJoin(old: ProsemirrorNode, start: number, end: number, $newStart: ResolvedPos, $newEnd: ResolvedPos) {
  if (
...
function findDiff(a: any, b: any, pos: number, preferredPos: number, preferredSide: "end" | "start") {
