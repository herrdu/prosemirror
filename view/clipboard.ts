import { Slice, Fragment, DOMParser, DOMSerializer, ResolvedPos, Node as ProsemirrorNode, NodeType } from "../model";
...
export function parseFromClipboard(
  ...
  else slice = Slice.maxOpen(normalizeSiblings(slice.content, $context), false);
  ...
  for (let d = $context.depth; d >= 0; d--) {
    ...
    fragment.forEach((node) => {
      ...
      let wrap = match.findWrapping(node.type),
        inLast: ProsemirrorNode;
      if (!wrap) return (result = null);
      if (
        (inLast = result.length && lastWrap.length && addToSibling(wrap, lastWrap, node, result[result.length - 1], 0))
      ) {
...
function withWrappers(node: ProsemirrorNode, wrap: Array<NodeType> | null | undefined, from = 0) {
...
function closeRight(node: ProsemirrorNode, depth: number) {
