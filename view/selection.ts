...
export function selectionFromDOM(view: EditorView, origin: string) {
  ...
  if (selectionCollapsed(domSel)) {
    ...
    if (
      ...
      NodeSelection.isSelectable(nearestDesc.node) &&
      nearestDesc.parent &&
...
function setEditable(element: any) {
...
function resetEditable(element: any) {
...
function removeClassOnSelectionChange(view: EditorView) {
  ...
  doc.removeEventListener("selectionchange", (view as any).hideSelectionGuard);
  let domSel = view.root.getSelection();
  let node = domSel.anchorNode,
    offset = domSel.anchorOffset;
  doc.addEventListener(
    "selectionchange",
    ((view as any).hideSelectionGuard = () => {
...
export function hasFocusAndSelection(view: EditorView) {
  if (view.editable && (view.root as Document).activeElement != view.dom) return false;
...
export function hasSelection(view: EditorView) {
  ...
  try {
    ...
    // element.
    return (
      view.dom.contains(sel.anchorNode.nodeType == 3 ? sel.anchorNode.parentNode : sel.anchorNode) &&
