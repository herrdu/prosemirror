import { Slice, Schema, Node as ProsemirrorNode } from "../model";
...
export class ReplaceStep<S extends Schema = any> extends Step<S> {
  ...
  toJSON() {
    let json: { [key: string]: any } = { stepType: "replace", from: this.from, to: this.to };
...
export class ReplaceAroundStep extends Step {
  ...
  apply(doc: ProsemirrorNode) {
    ...
    let inserted = (this.slice as any).insertAt(this.insert, gap.content);
  ...
  invert(doc: ProsemirrorNode) {
    ...
    return new ReplaceAroundStep(
      ...
      (doc.slice(this.from, this.to) as any).removeBetween(this.gapFrom - this.from, this.gapTo - this.from),
      this.gapFrom - this.from,
      this.structure
    );
  }
...
function contentBetween(doc: ProsemirrorNode, from: number, to: number) {
