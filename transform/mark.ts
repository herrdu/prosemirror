import { MarkType, Slice, Fragment, Mark, NodeType, ContentMatch, Node as ProsemirrorNode } from "../model";
...
Transform.prototype.addMark = function (from: number, to: number, mark: Mark) {
  let removed = [],
    added = [],
    removing = null,
    adding = null;
  this.doc.nodesBetween(from, to, (node: any, pos: number, parent: ProsemirrorNode) => {
...
// any type.
Transform.prototype.removeMark = function (from: number, to: number, mark: Mark = null) {
  let matched = [],
    step = 0;
  this.doc.nodesBetween(from, to, (node: ProsemirrorNode, pos: any) => {
    if (!node.isInline) return;
    step++;
    let toRemove = null;
...
// argument.
Transform.prototype.clearIncompatible = function (
  pos: number,
  parentType: NodeType,
