import { Slice, Fragment, NodeRange, Node as ProsemirrorNode, NodeType, Schema, Mark } from "../model";

import { Transform } from "./transform";
import { ReplaceStep, ReplaceAroundStep } from "./replace_step";

function canCut(node: ProsemirrorNode, start: number, end: number) {
...
export function liftTarget(range: NodeRange) {
  let parent = range.parent;
  let content = (parent.content as any).cutByIndex(range.startIndex, range.endIndex);
...
Transform.prototype.lift = function (range: NodeRange, target: number): Transform {
...
Transform.prototype.wrap = function (
  range: NodeRange,
...
Transform.prototype.setBlockType = function (
  from: number,
  to: number | undefined,
  type: NodeType,
  attrs?: { [key: string]: any }
...
Transform.prototype.setNodeMarkup = function (
  pos: number,
  type?: NodeType,
  attrs?: { [key: string]: any },
  marks?: Array<Mark>
...
export function canSplit<S extends Schema = any>(
  ...
  if (
    ...
    !innerType.type.validContent(($pos.parent.content as any).cutByIndex($pos.index(), $pos.parent.childCount))
  )
    return false;
  for (let d = $pos.depth - 1, i = depth - 2; d > base; d--, i--) {
    ...
    let rest = (node.content as any).cutByIndex(index, node.childCount);
...
// use after the split.
Transform.prototype.split = function (
  pos: number,
  depth: number = 1,
  typesAfter?: Array<{ type: NodeType; attrs?: { [key: string]: any } | null }>
) {
...
// joined.
export function canJoin(doc: ProsemirrorNode, pos: number) {
  let $pos = doc.resolve(pos),
    index = $pos.index();
  return joinable($pos.nodeBefore, $pos.nodeAfter) && $pos.parent.canReplace(index, index + 1);
}

function joinable(a: any, b: any) {
...
// point, if any.
export function joinPoint(doc: ProsemirrorNode, pos: number, dir: number = -1) {
  let $pos = doc.resolve(pos);
...
Transform.prototype.join = function (pos: number, depth: number = 1) {
...
export function insertPoint(doc: ProsemirrorNode, pos: number, nodeType: NodeType): number | undefined {
  let $pos = doc.resolve(pos);
...
export function dropPoint(doc: ProsemirrorNode, pos: number, slice: Slice) {
  let $pos = doc.resolve(pos);
