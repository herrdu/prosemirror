...
function recoverIndex(value: any) {
...
export class MapResult {
  ...
  constructor(pos: number, deleted: boolean = false, recover: null | any = null) {
...
// post-step version.
export class StepMap<S extends Schema = any> implements Mappable {
  ranges: number[];
  ...
  constructor(ranges: number[], inverted: boolean = false) {
    this.ranges = ranges;
  ...
  mapResult(pos: number, assoc: number = 1) {
    return this._map(pos, assoc, false) as MapResult;
  }

  // : (number, ?number) → number
  map(pos: number, assoc: number = 1) {
  ...
  touches(pos: number, recover: any) {
    let diff = 0,
...
export class Mapping implements Mappable {
  ...
  constructor(maps?: StepMap[], mirror?: any, from?: number, to?: number) {
    // :: [StepMap]
