import { Fragment, Slice, Node as ProsemirrorNode, ResolvedPos, NodeType, ContentMatch } from "../model";
...
export function replaceStep(doc: ProsemirrorNode, from: number, to: number = from, slice = Slice.empty) {
...
// given `slice`.
Transform.prototype.replace = function (from: number, to: number = from, slice = Slice.empty) {
...
class Fitter {
  ...
  constructor($from: ResolvedPos, $to: ResolvedPos, slice: Slice) {
    this.$to = $to;
    this.$from = $from;
    this.unplaced = slice;
      ...
      for (let sliceDepth = this.unplaced.openStart; sliceDepth >= 0; sliceDepth--) {
        ...
        for (let frontierDepth = this.depth; frontierDepth >= 0; frontierDepth--) {
          ...
          if (
            pass == 1 &&
            (first
              ? match.matchType(first.type) || (inject = match.fillBefore(Fragment.from(first), false))
  ...
  openFrontierNode(type: NodeType, attrs?: ProsemirrorNode["attrs"], content?: Fragment) {
    let top = this.frontier[this.depth];
...
function dropFromFragment(fragment: Fragment, depth: number, count: number) {
  if (depth == 0) return (fragment as any).cutByIndex(count);
...
function addToFragment(fragment: Fragment, depth: number, content: Fragment) {
...
function closeNodeStart(node: ProsemirrorNode, openStart: number, openEnd: number) {
...
function contentAfterFits($to: ResolvedPos, depth: number, type: any, match: ContentMatch, open: boolean) {
    ...
    for (let i = 0; i < targetDepths.length; i++) {
      ...
      if (parent.canReplaceWith(index, index, insert.type, insert.marks))
        return this.replace(
          ...
          new Slice(closeFragment(slice.content, 0, slice.openStart, openDepth), openDepth, slice.openEnd)
        );
    }
  }
...
function closeFragment(fragment: Fragment, depth: number, oldOpen: number, newOpen: number, parent?: ProsemirrorNode) {
  if (depth < oldOpen) {
    let first = fragment.firstChild;
