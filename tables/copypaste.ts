...
import { Slice, Fragment, Schema, NodeType } from "../model";
import { Transform, Mapping } from "../transform";

import { setAttr, removeColSpan } from "./util";
import { TableMap, Rect } from "./tablemap";
import { CellSelection } from "./cellselection";
import { tableNodeTypes } from "./schema";
import { Transaction, EditorState } from "../state";
...
// they stick out.
export function clipCells({ width, height, rows }, newWidth: number, newHeight: number) {
  if (width != newWidth) {
    let added = [],
      newRows = [];
