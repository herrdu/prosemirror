...
import { Selection, TextSelection, NodeSelection, SelectionRange, Transaction, EditorState } from "../state";
import { Decoration, DecorationSet } from "../view";
import { Fragment, Slice, ResolvedPos, Node as ProsemirrorNode } from "../model";
...
export class CellSelection extends Selection {
  ...
  constructor($anchorCell: ResolvedPos, $headCell: ResolvedPos = $anchorCell) {
    ...
    let cells = map.cellsInRect(rect).filter((p: any) => p != $headCell.pos - start);
    // Make the head cell the first range, so that it counts as the
    // primary part of the selection
    cells.unshift($headCell.pos - start);
    let ranges = cells.map((pos: any) => {
...
function isTextSelectionAcrossCells({ $from, $to }) {
  let fromCellBoundaryNode: any;
  let toCellBoundaryNode: any;
...
export function normalizeSelection(state: EditorState, tr: Transaction, allowTableNodeSelection: boolean) {
  let sel = (tr || state).selection,
    doc = (tr || state).doc,
    normalize: CellSelection | TextSelection,
    role: "cell" | "row" | "header_cell" | string;
