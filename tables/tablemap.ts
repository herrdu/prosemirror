...
let readFromCache: (key: any) => any, addToCache: (key: any, value: any) => any;
// Prefer using a weak map to cache table maps. Fall back on a
// fixed-size cache if that's not supported.
if (typeof WeakMap != "undefined") {
  let cache = new WeakMap();
  readFromCache = (key) => cache.get(key);
  addToCache = (key: any, value: any) => {
    cache.set(key, value);
    return value;
  };
} else {
...
export class TableMap {
  ...
  constructor(width: number, height: number, map: any, problems: any) {
    ...
    for (let i = 0; ; i++) {
      ...
      for (let h = 0; h < rowspan; h++) {
        ...
        for (let w = 0; w < colspan; w++) {
          ...
          else (problems || (problems = [])).push({ type: "collision", row, pos, n: colspan - w });
...
function findBadColWidths(map: any, colWidths: number[], table: ProsemirrorNode) {
...
function freshColWidth(attrs: any) {
