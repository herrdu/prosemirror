...
import { TextSelection, EditorState, Transaction } from "../state";
import { Fragment, Node as ProsemirrorNode, NodeType, ResolvedPos } from "../model";
...
export function addColumn(tr: Transaction, { map, tableStart, table }, col) {
  ...
  for (let row = 0; row < map.height; row++) {
    ...
    if (col > 0 && col < map.width && map.map[index - 1] == map.map[index]) {
      ...
      let type =
        refColumn == null ? tableNodeTypes(table.type.schema).cell : table.nodeAt(map.map[index + refColumn]).type;
...
export function addColumnBefore(state: EditorState, dispatch?: (tr: Transaction) => boolean) {
...
export function addColumnAfter(state: EditorState, dispatch?: (tr: Transaction) => boolean) {
  if (!isInTable(state)) return false;
  if (dispatch) {
    let rect = selectedRect(state);
...
export function removeColumn(tr: Transaction, { map, table, tableStart }, col: number) {
...
export function deleteColumn(state: EditorState, dispatch?: (tr: Transaction) => boolean) {
  if (!isInTable(state)) return false;
  if (dispatch) {
    let rect = selectedRect(state),
      tr = state.tr;
...
export function rowIsHeader(map: any, table: ProsemirrorNode, row: any) {
...
export function addRow(tr: Transaction, { map, tableStart, table }, row: any) {
...
export function removeRow(tr: Transaction, { map, table, tableStart }, row: any) {
  let rowPos = 0;
  for (let i = 0; i < row; i++) rowPos += table.child(i).nodeSize;
...
export function deleteRow(state: EditorState, dispatch?: (tr: Transaction) => boolean) {
  if (!isInTable(state)) return false;
  if (dispatch) {
    let rect = selectedRect(state),
      tr = state.tr;
...
function cellsOverlapRectangle({ width, height, map }, rect: Rect) {
...
export function splitCellWithType(
  getCellType: ({ row, col, node }: { row: number; col: number; node: ProsemirrorNode }) => NodeType
) {
  return (state: EditorState, dispatch?: (tr: Transaction) => boolean) => {
    let sel = state.selection;
    let cellNode: ProsemirrorNode, cellPos: number;
    if (!(sel instanceof CellSelection)) {
  ...
  return function (state: EditorState, dispatch?: (tr: Transaction) => boolean) {
    ...
    if (dispatch) {
      ...
      if (state.selection instanceof CellSelection)
        state.selection.forEachCell((node: ProsemirrorNode, pos: number) => {
...
function deprecated_toggleHeader(type: string) {
  return function (state: EditorState, dispatch?: (tr: Transaction) => boolean) {
    ...
    if (dispatch) {
      ...
      let nodes = cells.map((pos: number) => rect.table.nodeAt(pos));
      for (
        let i = 0;
        i < cells.length;
...
function isHeaderEnabledByType(type: "row" | "column", rect: any, types: any) {
...
export function deleteTable(state: EditorState, dispatch?: (tr: Transaction) => boolean) {
