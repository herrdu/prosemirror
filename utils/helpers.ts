import { NodeSelection, Transaction, Selection } from "../state";
import { Fragment, Node as PMNode, ResolvedPos, Schema, NodeType } from "../model";
import { TableMap } from "../tables";
...
export const equalNodeType = (nodeType: NodeType | NodeType[], node: PMNode) => {
...
export const replaceNodeAtPos = (position: number, content: Fragment | PMNode) => (tr: Transaction) => {
...
export const findTableClosestToPos = ($pos: ResolvedPos) => {
  const predicate = (node: PMNode) => node.type.spec.tableRole && /table/i.test(node.type.spec.tableRole);
  return findParentNodeClosestToPos($pos, predicate);
};

export const createCell = (cellType: NodeType, cellContent: Fragment = null) => {
  if (cellContent) {
...
export const isRectSelected = (rect: { left: number; right: number; top: number; bottom: number }) => (
  selection: Selection
) => {
  ...
  const selectedCells = map.cellsInRect(
    map.rectBetween((selection as any).$anchorCell.pos - start, (selection as any).$headCell.pos - start)
  );
...
// ```
//
export const convertArrayOfRowsToTableNode = (tableNode: PMNode, arrayOfNodes: Array<PMNode[]>) => {
  const rowsPM = [];
  const map = TableMap.get(tableNode);
  for (let rowIndex = 0; rowIndex < map.height; rowIndex++) {
...
export const moveTableColumn = (table: any, indexesOrigin: any, indexesTarget: any, direction: number) => {
...
export const moveTableRow = (table: any, indexesOrigin: any, indexesTarget: any, direction: number) => {
...
const moveRowInArrayOfRows = (rows: any[], indexesOrigin: any, indexesTarget: any, directionOverride: any) => {
...
export const checkInvalidMovements = (originIndex: number, targetIndex: number, targets: any[], type: any) => {
