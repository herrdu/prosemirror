import { CellSelection, TableMap, addColumn, addRow, removeColumn, removeRow } from "../tables";
...
// ```
export const getSelectionRect = (selection: CellSelection) => {
  if (!isCellSelection(selection)) {
    return;
  }
...
// ```
export const isColumnSelected = (columnIndex: number) => (selection: CellSelection) => {
  if (isCellSelection(selection)) {
    const map = TableMap.get(selection.$anchorCell.node(-1));
...
export const moveRow = (originRowIndex, targetRowIndex, opts) => (tr) => {
  ...
  const { indexes: indexesOriginRow } = getSelectionRangeInRow(originRowIndex)(tr);
  const { indexes: indexesTargetRow } = getSelectionRangeInRow(targetRowIndex)(tr);
...
// );
// ```
export const moveColumn = (originColumnIndex, targetColumnIndex, opts) => (tr) => {
  const defaultOptions = { tryToFit: false, direction: 0 };
  const options = Object.assign(defaultOptions, opts);
  const table = findTable(tr.selection);
  if (!table) {
  ...
  const { indexes: indexesOriginColumn } = getSelectionRangeInColumn(originColumnIndex)(tr);
  const { indexes: indexesTargetColumn } = getSelectionRangeInColumn(targetColumnIndex)(tr);
