import { NodeSelection, Selection, Transaction } from "../state";
import { Node as ProseMirrorNode, Fragment, NodeType } from "../model";
...
// );
// ```
export const removeParentNodeOfType = (nodeType: NodeType | NodeType[]) => (tr: Transaction) => {
...
// );
// ```
export const replaceParentNodeOfType = (nodeType: NodeType | NodeType[], content) => (tr) => {
...
export const replaceSelectedNode = (content) => (tr) => {
  if (isNodeSelection(tr.selection)) {
    ...
    if (
      (content instanceof Fragment && $from.parent.canReplace($from.index(), $from.indexAfter(), content)) ||
...
// );
// ```
export const safeInsert = (content: ProseMirrorNode | Fragment, position?: number, tryToReplace?: boolean) => (
  tr: Transaction
) => {
